...
class PostgresLoader:
    ...
    def _prepare_buffer_from_df(self, df: pd.DataFrame) -> io.StringIO:
        ...
        df.to_csv(buffer, index=False, sep=",", header=False, na_rep="", quoting=csv.QUOTE_MINIMAL)
        ...
        try:
            ...
            with self._engine.begin() as conn:
                ...
                with raw_conn.cursor() as cursor:
                    ...
                    """).format(table_id, sql.SQL(", ").join(map(sql.Identifier, columns)))

                    logger.debug(f"Type of raw_conn: {type(raw_conn)}")
                    logger.debug(f"Type of cursor: {type(cursor)}")
